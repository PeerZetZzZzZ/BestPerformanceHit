...
class BestPerformanceHit:
    ...
    def process_file(self, input_file, execution_count):
        while True:
            single_host = input_file.readline()
            if len(single_host) == 0:
                sys.exit()
            else:
                self.__run_traceroute_paris_for_host(single_host, execution_count)

    def __run_traceroute_paris_for_host(self, host_address, count_of_traceroute_execution):
        for i in range(0, count_of_traceroute_execution):
